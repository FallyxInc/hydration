  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      ipcDataFiles.forEach((file, index) => {
        ...
        console.log(`📄 [FILE UPLOAD] Added IPC data file ${index}: ${file.name} (${file.size} bytes)`);
      });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            {ipcDataFiles.length > 0 && (
              <div className="mt-2">
                <p className="text-sm text-green-600 font-medium">Selected {ipcDataFiles.length} file(s):</p>
                <ul className="mt-1 text-sm text-gray-600">
