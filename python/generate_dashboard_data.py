...
"""
        ...
        print(f"✅ Generated {js_data_path} with {len(residents_data)} residents (date: {date_col})")
        ...
        if duplicates:
            ...
            for name, entries in name_groups.items():
                if len(entries) == 1:
                    ...
                    print(f"   ✓ Merged into single entry with goal={best_entry.get('goal', 0)}, data={best_entry.get('data', 0)}")
...
"""
        ...
        # Recalculate stats
        goal_met = sum(1 for r in final_data if r['goal'] > 0 and r.get('data', 0) >= r['goal'])
